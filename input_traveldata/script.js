...
const button = document.getElementById('kakunin');//確認ボタン(押下することでチェックボックスのチェック状況を取得することができる)
...
items.forEach(item => {
    ...
    label.textContent = item;
        ...
        // 何かわからないがなくもエラーにはならない(逆にあるとエラーになるため注意)
        // checkbox.addEventListener('change', () => {
        //   checkboxStates[checkboxStates.indexOf(checkbox.value)] = checkbox.checked;
        // });
    
    content.appendChild(checkbox);
    content.appendChild(label);
    content.appendChild(document.createElement('br')); // 改行
